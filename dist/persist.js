...
if (CREDENTIALS_KEY && CREDENTIALS_KEY.length < 32) {
    log.warn({ length: CREDENTIALS_KEY.length }, "[persist] OPENWA_CREDENTIALS_KEY is shorter than 32 chars — recommended: openssl rand -hex 32");
}
let pool = null;
/** Effective key material for credentials encryption. */
function credentialsKeyMaterial() {
...
export async function loadCreds(name) {
    try {
        ...
        if (res.rowCount === 0)
            return null;
        const blob = res.rows[0].creds;
        try {
            return decrypt(blob);
        }
        catch (currentKeyErr) {
            if (!legacyKeyAvailable()) {
                log.warn({ err: currentKeyErr, name }, "[persist] decrypt failed with the current key — treating as absent");
                return null;
            }
            let plaintext;
            try {
                ...
                log.warn({ err: legacyErr, name }, "[persist] decrypt failed with BOTH current and legacy key — treating as absent");
                return null;
            }
            ...
            try {
                await p.query(`UPDATE openwa_sessions SET creds = $2, updated_at = NOW() WHERE name = $1`, [name, encrypt(plaintext)]);
                log.info({ name }, "[persist] re-keyed legacy credentials blob to OPENWA_CREDENTIALS_KEY");
            }
            catch (rekeyErr) {
                // Re-key write failed (transient DB issue): still return the
                // plaintext — the next save writes it with the current key anyway.
                log.warn({ err: rekeyErr, name }, "[persist] re-key UPDATE failed — returning plaintext; next save will re-encrypt");
            }
            return plaintext;
        }
    }
