      ...
      if (hasCreds) {
        try {
          ...
          localRegistered = Boolean(
            (JSON.parse(raw) as { registered?: unknown } | null)?.registered,
          );
        } catch {
        ...
        if (blob) {
          if (hasCreds) {
            ...
            log.warn(
              ...
              "[persist] interim local credentials replaced by the DB blob (O2-F1)",
            );
          }
  ...
  sock.ev.on("connection.update", (update) => {
    ...
    if (connection === "close") {
      ...
      if (rs.socket !== sock) {
        log.warn(
          { sessionId: rs.id, name: rs.name },
          "[session] stale close from a superseded socket ignored",
        );
        return;
      }
    ...
    for (const name of names) {
      ...
      if (!SESSION_NAME_RE.test(name)) {
        log.warn(
          ...
          "[boot] persisted session name failed SESSION_NAME_RE — skipped (path-traversal guard)",
        );
        continue;
      }
      const rs: RuntimeSession = {
